...
public class Assignment3 {
	...
	/*
	 * overall time complexity : o(N * N). space complexity : o(1)
	 */
	public static List<List<Integer>> threeSumZero(int[] arr) {
		...
		Arrays.sort(arr);
		// The time Complexity is O(nlogn) sorted the whole array

		for (int i = 0; i < arr.length; i++) { // this loop run N time
			if (i > 0 && arr[i] == arr[i - 1])
				continue; // Reduced duplicates number

			int left = i + 1; // first pointer start with i +1
			int right = arr.length - 1;// second pointer start with end
			while (left < right) {
				// using two pointers, this loop run N time. The time time
				// complexity is O(n^2)
				// compare sum of every element with fixed i
				if (arr[i] + arr[left] + arr[right] == 0) {
					...
					left++;
					right--;
				} else if (arr[i] + arr[left] + arr[right] > 0) { // move right
																	// pointer
																	// to left
																	// if sum is
																	// positive
	...
	public static int longestSubstring(String s) {
		if (s == null || s.length() == 0)
			return -1; // check if string is valid
		int max = 0;
		int index = 0;
		...
		for (int i = 0, index1 = 0; i < s.length(); i++) { // this loop run N
															// time. The time
															// time complexity
															// is O(n)
			char c = s.charAt(i);
			if (map.containsKey(c)) {// when duplicate char detected, the index
										// is updated
				index1 = Math.max(index, map.get(c) + 1);
			}
			map.put(c, i);// skip the repeated character
			max = Math.max(max, i - index1 + 1);// the case that the substring
